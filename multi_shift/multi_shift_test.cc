...
int main(int argc, char **argv)
{
    ...
    LatticeInt shftd_coord_in_dir = multi_shift<LatticeInt>(coord_in_dir, steps, dir, nrow[dir], +1);//put +1 just for test

    LatticeDouble shft_phs_in_dir = one;
    if (bndry_cndtns[dir] == 1) //periodic boundary condition
    {
        ;
    }
    else if (bndry_cndtns[dir] == -1)
    {
        if (steps > 0)
        {
    ...
    multi_shift_file.push(to_string(steps) + "steps_in_" + to_string(dir));
    ...
    multi_shift_file.pop();

    multi_shift_file.close();
    cout << "================================================\n"
         << endl;
    cout << "bndry cntns:" << bndry_cndtns[0] << " " << bndry_cndtns[1] << " "
         << bndry_cndtns[2] << " " << bndry_cndtns[3] << "\n"
         << endl;

    cout << to_string(steps) + "steps_in_" + to_string(dir) << endl;
